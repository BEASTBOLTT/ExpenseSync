...
/**
 * @desc Get All Spaces (that the user is a member of)
...
/**
 * @desc Update Space Expense (non-financial fields only)
...
async function deleteSpaceExpense(req, res) {
    ...
    if (space.createdBy.toString() !== account._id.toString()) {
        return res.status(403).json({
            message: "Only the creator can delete space expenses",
            status: "failed"
        })
    }

    const expense = await spaceExpenseModel.findByIdAndDelete(expenseId)
